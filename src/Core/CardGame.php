...
class CardGame
{
   ...
   */
    private array $jeuCartes;

    /**
   * Guess constructor.
   * @param array $cards
   */
    public function __construct(array $jeudeCartes)
    {
        $this->jeuCartes = $jeudeCartes;
    }
   ...
   */
    public function shuffle()
    {
        ...
        $newCards = shuffle($this->jeuCartes);
   ...
   */
    ...
    {
        $orderCard = usort($this->jeuCartes, array($this, "compare"));
        return $this->jeuCartes;
    }
   ...
   */
    ...
    */
    public static function factory32Cards() : array {
        // TODO création d'un jeu de 32 cartes /*CREATION DU JEUX DE 32 XCARTES */
        $cards = array();
        foreach (self::ORDER_COLORS as $color) {
            foreach (self::ORDER_NAME as $name) {
                if($name >= 6){
    ...
    public function getCard(int $index) : Card {
        if ($index >= 0 && $index <= count($this->jeuCartes)) {
            return  $this->jeuCartes[$index];
        }
    }
  ...
  {
      return 'CardGame : '.count($this->jeuCartes).' carte(s)';
  }
